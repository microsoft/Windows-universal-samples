...
void Scenario1_DataEvents::VisibilityChanged(Object^ sender, VisibilityChangedEventArgs^ e)
{
    ...
    if (ScenarioDisableButton->IsEnabled)
    {
        if (e->Visible)
        {
            ...
            readingToken = sensor->ReadingChanged::add(ref new TypedEventHandler<LightSensor^, LightSensorReadingChangedEventArgs^>(this, &Scenario1_DataEvents::ReadingChanged));
        }
        else
        {
...
void Scenario1_DataEvents::ReadingChanged(LightSensor^ sender, LightSensorReadingChangedEventArgs^ e)
{
