...
(function () {
    ...
    var printLineTextBox;
    var barcodeTextBox;

    var page = WinJS.UI.Pages.define("/html/Scenario2_PrintReceipt.html", {

        ready: function (element, options) {
            printLineTextBox = document.getElementById("printLineTextBox");
            barcodeTextBox = document.getElementById("barcodeTextBox");

            document.getElementById("printLineButton").addEventListener("click", printLine);
            document.getElementById("printSampleReceiptsButton").addEventListener("click", printReceipt);
            document.getElementById("printBitmapButton").addEventListener("click", printBitmap);
            document.getElementById("printBarcodeButton").addEventListener("click", printBarcode);
        }
    });
    ...
    function printBarcode() {
        ...
        var job = SdkSample.claimedPrinter.receipt.createJob();
        job.printBarcode(barcodeTextBox.value, BarcodeSymbologies.upca, 60, 3, PosPrinterBarcodeTextPosition.below, PosPrinterAlignment.center);
        executeJobAndReportResultAsync(job).done();
    }
})();
