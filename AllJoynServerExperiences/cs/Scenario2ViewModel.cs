...
namespace AllJoynServerExperiences
{
    class Scenario2ViewModel : INotifyPropertyChanged
    {
        ...
        private async void OnboardingService_ConnectRequestRecieved(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (foundSsid)
                {
                    if (supportedProtocol)
                    {
                        ...
                        {
                            if (AppData.OnboardingConfigurePassphrase.Equals(AppData.SampleNetworkPassword))
                            {
                                returnArg.Value1 = (short)ConnectionResultCode.Validated;
                                returnArg.Value2 = "Connected successfully";
                                AppData.OnboardingConfigurationState = (short)ConfigurationState.Validated;
            ...
            {
                UpdateStatusAsync("Successfully connected to " + AppData.OnboardingConfigureSsid, NotifyType.StatusMessage);
            }
            else
            {
                UpdateStatusAsync(string.Format("Attempt to connect failed with result code: {0} and message: {1}.", ((ConnectionResultCode)returnArg.Value1).ToString(), returnArg.Value2), NotifyType.ErrorMessage);
            }
        }
