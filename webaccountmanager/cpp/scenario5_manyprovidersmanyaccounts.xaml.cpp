...
{
    ApplicationData::Current->LocalSettings->CreateContainer((String^) ACCOUNTS_CONTAINER, ApplicationDataCreateDisposition::Always);
    ApplicationData::Current->LocalSettings->Containers->Lookup((String^) ACCOUNTS_CONTAINER)->CreateContainer((String^) PROVIDER_ID_SUBCONTAINER, ApplicationDataCreateDisposition::Always);
    ApplicationData::Current->LocalSettings->Containers->Lookup((String^) ACCOUNTS_CONTAINER)->CreateContainer((String^) AUTHORITY_SUBCONTAINER, ApplicationDataCreateDisposition::Always);
}
...
void SDKTemplate::Scenario5_ManyProvidersManyAccounts::GetAccounts()
{
    ...
    ApplicationDataContainer^ accountListContainer = ApplicationData::Current->LocalSettings->Containers->Lookup((String^) ACCOUNTS_CONTAINER);

    for(Object^ value : accountListContainer->Containers->Lookup((String^) PROVIDER_ID_SUBCONTAINER)->Values)
    {
        String^ accountID = (String^)value;
        String^ providerID = (String^)accountListContainer->Containers->Lookup((String^) PROVIDER_ID_SUBCONTAINER)->Values->Lookup(accountID);
        String^ authority = (String^)accountListContainer->Containers->Lookup((String^) AUTHORITY_SUBCONTAINER)->Values->Lookup(accountID);
...
{
    ApplicationDataContainer^ accountsContainer = ApplicationData::Current->LocalSettings->Containers->Lookup((String^) ACCOUNTS_CONTAINER);
    ...
    if (account->Id != "")
    {
        accountsContainer->Containers->Lookup((String^) PROVIDER_ID_SUBCONTAINER)->Values->Insert(account->Id, account->WebAccountProvider->Id);
        accountsContainer->Containers->Lookup((String^) AUTHORITY_SUBCONTAINER)->Values->Insert(account->Id, account->WebAccountProvider->Authority);
        m_accounts->Insert(account->UserName, account);
    }
    else
    {
        accountsContainer->Containers->Lookup((String^) PROVIDER_ID_SUBCONTAINER)->Values->Insert(account->UserName, account->WebAccountProvider->Id);
...
IAsyncAction^ SDKTemplate::Scenario5_ManyProvidersManyAccounts::LogoffAndRemoveAccount(WebAccount^ account)
{
    ...
    if (account->WebAccountProvider->Id != APP_SPECIFIC_PROVIDER_ID)
    {
        ...
        accountsContainer->Containers->Lookup((String^) PROVIDER_ID_SUBCONTAINER)->Values->Remove(account->Id);
        accountsContainer->Containers->Lookup((String^) AUTHORITY_SUBCONTAINER)->Values->Remove(account->Id);
    ...
    else
    {
        ...
        accountsContainer->Containers->Lookup((String^) PROVIDER_ID_SUBCONTAINER)->Values->Remove(account->UserName);
