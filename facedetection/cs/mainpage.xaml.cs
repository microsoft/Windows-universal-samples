...
namespace FaceDetection
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        /// <summary>
        /// Out of all preview resolutions with the highest pixel count, it will return the one with the highest framerate
        /// </summary>
        /// <returns></returns>
        private VideoEncodingProperties FindBestPreviewResolution()
        {
            // Find all preview resolutions
            var resolutions = _mediaCapture.VideoDeviceController.GetAvailableMediaStreamProperties(MediaStreamType.VideoPreview).Select(x => x as VideoEncodingProperties);

            // Sort by highest pixel count first, then by highest framerate
            return resolutions.OrderByDescending(x => x.Width * x.Height).ThenByDescending(x => x.FrameRate.Numerator / (double)x.FrameRate.Denominator).FirstOrDefault();
        }
