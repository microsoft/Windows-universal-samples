...
# XAML and Direct3D view switching example

Shows how to switch between an XAML view of an app, and an exclusive Direct3D view
...
The application starts up with an XAML view of the application, which contains a button to launch
the Holographic environment. If it isn't detected, the button is unavailable.

When in the exclusive view, this sample uses a device-attached frame of reference with stereoscopic 
billboarding, linear interpolation, velocity estimation, and image stabilization to render a 
holographic message that the user can always comfortably see.
...
is always visible to the user. The user can switch back to the XAML by performing an air tap gesture.
