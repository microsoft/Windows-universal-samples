    ...
    m_d3dRenderTargetSize(camera->RenderTargetSize)
{
    ...
    m_holographicCamera->SetNearPlaneDistance(m_nearPlaneDistance);
    m_holographicCamera->SetFarPlaneDistance(m_farPlaneDistance);

    InitDepthCurveArray(
        m_nearPlaneDistance,
        m_farPlaneDistance,
        m_nearPlaneDistance > m_farPlaneDistance ? 0 : UINT16_MAX,
        m_depthWeightArray);
};
    ...
    )
{
    ...
    if (m_d3dDepthStencilView == nullptr)
    {
        ...
        // Create shader resource view for depth buffer, so we can resolve it in a compute shader.
        ...
        DX::ThrowIfFailed(
            device->CreateShaderResourceView(depthStencil.Get(), &depthSRVDesc, &m_d3dDepthShaderResourceView)
            );
        ...
        int depthW = lround(m_d3dRenderTargetSize.Width);
        int depthH = lround(m_d3dRenderTargetSize.Height);
        const unsigned int outWidth  = depthW / DEPTH_BUFFER_SUBSAMPLE_RATE;
        const unsigned int outHeight = depthH / DEPTH_BUFFER_SUBSAMPLE_RATE;
        ...
        if (depthBufferMappedOnGpu)
        {
            ...
            DX::ThrowIfFailed(
                device->CreateBuffer(&depthResolvedDesc, nullptr, &m_d3dResolvedDepthBufferMappable)
                );
            ...
            DX::ThrowIfFailed(
                device->CreateUnorderedAccessView(
                    m_d3dResolvedDepthBufferMappable.Get(),
                    &depthResolvedUAVDesc,
                    &m_d3dResolvedDepthViewMappable)
                );
        }
        else
        {
            ...
            DX::ThrowIfFailed(
                device->CreateBuffer(&depthResolvedDesc, nullptr, &m_d3dResolvedDepthBuffer)
                );
            ...
            DX::ThrowIfFailed(
                device->CreateUnorderedAccessView(m_d3dResolvedDepthBuffer.Get(), &depthResolvedUAVDesc, &m_d3dResolvedDepthView)
                );
            ...
            DX::ThrowIfFailed(
                device->CreateBuffer(&depthResolvedCPUDesc, nullptr, &m_d3dCPUResolvedDepthTexture)
                );
        }
    }
    ...
    )
{
    ...
    m_projection = cameraProjectionTransform.Left;
    ...
    if (viewTransformAcquired)
    {
        ...
        auto viewInverse = XMMatrixInverse(&determinant, XMLoadFloat4x4(&viewCoordinateSystemTransform.Left));
        ...
        m_view = viewCoordinateSystemTransform.Left;
    }
