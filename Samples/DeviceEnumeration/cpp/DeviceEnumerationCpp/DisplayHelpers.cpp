...
namespace SDKTemplate
{
    ...
    IVectorView<DeviceSelectorInfo^>^ DeviceSelectorChoices::Selectors::get()
    {
        ...
        selectors->Append(ref new DeviceSelectorInfo("Video Casting Devices", DeviceClass::All, CastingDevice::GetDeviceSelector(CastingPlaybackTypes::Video)));
        // Using an empty string for the app name for the DIAL selector which returns all DIAL devices. For typical scenarios you would specify the App name.
        selectors->Append(ref new DeviceSelectorInfo("DIAL Devices (All apps)", DeviceClass::All, DialDevice::GetDeviceSelector("")));
