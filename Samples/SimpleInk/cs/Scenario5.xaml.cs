...
{
    /// <summary>
    /// This page shows the code to manipulate wet ink strokes.
    /// </summary>
    public sealed partial class Scenario5 : Page
    {
        ...
        // discontinueStroke is true if the Ink input that follows for the wet stroke is to be ignored
            ...
            if (!discontinueStroke)
            {
                for (int i = 0; i < args.NewInkPoints.Count; i++)
                {
                    ...
                    if (InCircleEdgeZone(distanceFromCenter))
                    {
                        ...
                        // Move the point to the calculated position.
        ...
        private void HandleRegularInput(CoreWetStrokeUpdateEventArgs args)
        {
            for (int i = 0; i < args.NewInkPoints.Count; i++)
            {
                ...
                if (distanceFromCenter < radius + circleStrokeThickness / 2 + penSize / 2)
                {
                    ...
                    // and completing the stroke. (Moving the last point to the circle edge
                    // rather than throwing it away is left as an exercise.)
